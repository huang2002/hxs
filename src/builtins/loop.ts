...
export const builtinFor: RuleHandler = Common.injectHelp(
    ...
    (rawArgs, context, env): RuleHandler => {
        ...
        return (rawBlock, ctx, _env) => {
            ...
            ctx.set('break', Common.injectHelp(
                'break()',
                () => { throw breakFlag; }
            ));
            ctx.set('continue', Common.injectHelp(
                'continue()',
                () => { throw continueFlag; }
            ));
            try {
...
export const builtinWhile: RuleHandler = Common.injectHelp(
    'while (condition) {...}',
    (rawArgs): RuleHandler => (
        (rawBlock, ctx, env) => {
